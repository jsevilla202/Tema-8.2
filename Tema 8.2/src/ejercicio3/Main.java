...
public class Main {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i<edi.length+poli.length; i++) {
			if(i<5) {
				System.out.println("La superficie del polideportivo es de " + poli[i].getSuperficieEdificio() + "m2 y el tipo es " + poli[i].getTipoDeInstalacion());
			}
			if(i<10) {
				System.out.println("El numero de oficinas es de " + edi[i-5].getNumoficinas() + " y la superficie del edificio es de " + edi[i-5].getSuperficieEdificio() + "m2");
			}
		}
	}
